...
class Venue(db.Model):
    ...
    def complete(self):
        past_shows = get_past_venue_shows(self.id)
        upcoming_shows = get_upcoming_venue_shows(self.id)
        return {
            'id': self.id,
            'name': self.name,
            'genres': self.genres,
...
class Artist(db.Model):
    ...
    #this can be calculated does not need to be a column
    #upcoming_shows_count = db.Column(db.Integer)
    #past_shows, past_shows_count

    # TODO: implement any missing fields, as a database migration using Flask-Migrate
    def complete(self):
        past_shows = get_past_artist_shows(self.id)
        upcoming_shows = get_upcoming_artist_shows(self.id)
        return {
            'id': self.id,
            'name': self.name,
            'genres': self.genres,
...
def create_venue_submission():
  ...
  # TODO: modify data to be the data object returned from db insertion
    error = False
    try:
        #assigning entire form to data dictionary
        data = request.form.to_dict()
        #this passes dict to class. the keys have to match the class model
        venue = Venue(**data)
        #this adds that object to the current session
        db.session.add(venue)
        db.session.commit()
    except:
        db.session.rollback()
        #set error so can check before return whether or not to return
        error = True
        print(sys.exc_info())
    finally:
        db.session.close()
...
def artists():
  ...
  #need to revisit this, this seems shitty but it is dynamic so maybe its great who cares.
  data = []
  for id, name in db.session.query(Artist.id, Artist.name):
      b = {}
      b['id'] = id
      b['name'] = name
      data.append(b)
